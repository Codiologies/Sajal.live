...
const BlogClientPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      ...
      try {
        ...
        const mappedPosts: BlogPost[] = (fetched || []).map((item: any) => ({
          _id: item._id || item.link,
          title: item.title,
          slug: {
            current: item.link ? encodeURIComponent(item.link.split('/').pop() || item.link) : '',
          },
          excerpt: item.excerpt,
          mainImage: item.image
            ? {
                asset: { url: item.image },
                alt: item.title,
              }
            : undefined,
          publishedAt: item.publishedAt,
          estimatedReadingTime: item.readingTime,
          categories: (item.tags || []).map((t: string) => ({ title: t, slug: { current: t.toLowerCase().replace(/\s+/g, '-') } })),
          tags: item.tags || [],
          author: { name: item.author || 'Sajal Gupta' },
          link: item.link,
        }));

        setPosts(mappedPosts);
        const allCategories = mappedPosts.flatMap((post: BlogPost) =>
  ...
  return (
    <>
      <Head>
        <title>Blog - Sajal Gupta | Cybersecurity Insights & Research</title>
        <meta name="description" content="Latest cybersecurity insights, bug bounty findings, and security research by Sajal Gupta. Stay updated with the latest in ethical hacking and penetration testing." />
            ...
            {featuredPost && (
              <section className="px-4 mb-12">
                <div className="container mx-auto max-w-6xl">
                  ...
                  >
                    <a
                      href={featuredPost.link || (featuredPost.slug?.current ? `/blog/${featuredPost.slug.current}` : '#')}
                      target={featuredPost.link ? "_blank" : undefined}
                  ...
                  <>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                      {filteredPosts.map((post, index) => (
                        ...
                        >
                          <a
                            href={post.link || (post.slug?.current ? `/blog/${post.slug.current}` : '#')}
